...
public class JinikExcelFileUpdateBatch {
	...
	public static void main( String[] args ) {
		...
		try {
			...
			catMsg.info( "取込みバッチが起動しました。 >> " + fmt.format( new Date()));

			propMap = getBatchProperties();

			// バッチ実行用プロパティの取得
			String batStopFilePath = propMap.get( "UPLOAD_BATCH_STOP" );
			long interval = Long.parseLong(propMap.get( "BATCH_THREAD_INTERVAL" ));

			// バッチ停止ファイルがあれば、削除
			File sf = new File( batStopFilePath );
			if ( sf.exists()) {
				sf.delete();
			}
			...
			while( bLoop ) {
				...
				File stopFile = new File( batStopFilePath );
				if ( stopFile.exists()) {
					catMsg.info( "取り込みバッチを終了します。 >> " + fmt.format( new Date()));
					break;
				}
	...
	private void execSendBatchMailTransaction( ZzJinikUploadReserveDto dto, JinikUpdateResultBean rsltBean, String mailType ) throws SQLException, IOException {
		...
		try {
			...
			replaceMap.put( "sender_name", userinfo.get( "PERSON_NAME" ) == null ? "" : userinfo.get( "PERSON_NAME" ));
			replaceMap.put( "file_name", dto.getUploadFileName() == null ? "" : dto.getUploadFileName());
			...
			mailDto.setToAddress( mailToAddr );
	...
	private MailInfoDto makeMailInfoDto( String mailType, ZzJinikUploadReserveDto rsvDto, Connection conn ) throws SQLException {
		...
		try {
			...
			mailBody = mailBody.replaceAll( "\\#\\{file_name\\}", rsvDto.getUploadFileName() == null ? "" : rsvDto.getUploadFileName());
			
			dto.setFromAddress( propMap.get( "BATCH_MAIL_FROM" ) == null ? "" : propMap.get( "BATCH_MAIL_FROM" ));
			dto.setToAddress( userinfo.get( "MAIL_ADDRESS" ) == null ? "" : userinfo.get( "MAIL_ADDRESS" ));
	...
	private static Map<String, String> getBatchProperties() throws IOException {
